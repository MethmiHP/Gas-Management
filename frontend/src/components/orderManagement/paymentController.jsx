...
// In PaymentController.jsx - Let's modify the CheckoutFlow component to include email option
...
// PaymentForm and AddressForm components remain the same...
...
const CheckoutFlow = () => {
  ...
  const handleCompleteOrder = async (address) => {
    // Store order details in localStorage for email page
    const confirmationId = 'ORDER-' + Math.floor(Math.random() * 1000000);
    
    // Save delivery details for email page
    ...
    localStorage.setItem('lastOrderConfirmation', JSON.stringify({
      ...
      amount: cart.total,
      paymentMethod: paymentInfo.method === 'Card' ? 'Credit Card' : 'Cash on Delivery'
    }));
